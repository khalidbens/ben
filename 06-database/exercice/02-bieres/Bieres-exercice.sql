...
FROM ventes
WHERE ID_ARTICLE IN (SELECT ID_ARTICLE FROM ventes WHERE ANNEE = 2014 AND NUMERO_TICKET = 856);
...
FROM ventes
WHERE ANNEE = 2016
AND QUANTITE <= (SELECT MAX(QUANTITE) * 0.85 FROM ventes WHERE ANNEE = 2016);
...
DELETE FROM ticket
WHERE id_ticket NOT IN (SELECT id_ticket FROM ventes);

--j'ai une base de donnée nommé "biere", j'ai plusieurs tables qui sont:" article", "continent", "couleur", "fabriquant", "marque", "pays", "ticket", "type" et "ventes".
